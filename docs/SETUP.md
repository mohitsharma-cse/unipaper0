...
- On Azure App Service, use Node 22 and startup command `npm start`.
...
- For this combined App Service deployment, use `COOKIE_SAME_SITE=lax` and `COOKIE_SECURE=true`. Use `COOKIE_SAME_SITE=none` only if the frontend and backend are on separate HTTPS origins.
